...
import { AlertTriangle, Shield, Bell, BellOff } from "lucide-react";
...
export default function LiveAlertsBanner({ onAlert, myCity }: Props) {
  ...
  // Ref so the fetch callback always sees the latest myCity without re-creating the interval
  ...
  const fetchAlert = useCallback(async () => {
    try {
      ...
      if (newId && newId !== prevAlertId.current) {
        ...
        // Only alert if: no city filter set, OR the alert includes our city
        const city = myCityRef.current;
        const relevant = !city || data!.data.includes(city);
  ...
  // Is our city in the active alert?
    ...
    <div className="ml-auto flex items-center gap-2 shrink-0">
      {notifEnabled && myCity && (
        <span className="text-xs text-gray-500 hidden sm:block">
          Monitoring <span className="text-gray-300">{myCity}</span>
        </span>
      )}
      <button
